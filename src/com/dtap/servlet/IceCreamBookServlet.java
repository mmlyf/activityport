...
/**
 * Servlet implementation class IceCreamBookServlet
 */
@WebServlet("/icebook")
public class IceCreamBookServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		log.info("号码："+phone+"提交冰激凌预约！");
		JSONObject json = new JSONObject();
		if(dsjIceDataBean.getDx_dn()!=null) {
			boolean result = IceCreamDbOperation.addIceDsjBookData(dsjIceDataBean);
			if(result) {
				log.info("号码："+phone+"提交冰激凌预约成功！");
				json.put("code", 0);
			}else {
				log.info("号码："+phone+"提交冰激凌预约失败！");
				json.put("code", 2);
			}
		}else {
