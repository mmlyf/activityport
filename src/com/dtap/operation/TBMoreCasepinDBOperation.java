 ...
 *
 */
public class TBMoreCasepinDBOperation {
	/**
	 * 查询当前时间是否有PV值，并将值返回
	 * @param addtime
	 * @return
	...
	public static Integer selectPVCountDataByAddtime(String addtime) {
		...
		String sql = "select * from tb_more_casepin where addtime = '"+addtime+"'";
		try {
	...
	public static boolean insertPvCountDataByAddtime(String addtime) {
		int inres = 0;
		Connection conn = GetConnect.getAliConnection();
		String sql = "insert into tb_more_casepin(pv,addtime) values(1,'"+addtime+"')";
		try {
			Statement statement = conn.createStatement();
	...
	public static boolean updatePvCountDataByAddtime(int pvvalue,String addtime) {
		int upres = 0;
		Connection conn = GetConnect.getAliConnection();
		String sql = "update tb_more_casepin set pv = "+pvvalue+" where addtime = '"+addtime+"'";
		try {
			Statement statement = conn.createStatement();
